...
def load_score():
    return webapp.load_score()


def save_score(score):
    webapp.save_score(score)


class Window:
    def __init__(self):
        self.loader = webgl.Loader()
        self.size = (1920, 1080)
    ...
    def update(self):
        self.mouse = webapp.mouse()

    def key_pressed(self, key):
        return webapp.key_pressed(self.keymap[key])

    def key_down(self, key):
        return webapp.key_down(self.keymap[key])


def load_audio(name):
    import wave
    with wave.open(io.BytesIO(assets['Audio'][name])) as w:
        return webapp.load_audio(w.readframes(w.getnframes()))
...
class Speaker:
    ...
    def play_intro(self):
        webapp.play_audio(self.intro)

    def play_music(self):
        webapp.play_audio(self.music)

    def play_hover(self):
        webapp.play_audio(self.hover)

    def play_beam(self):
        webapp.play_audio(self.beam)

    def play_explosion(self):
        webapp.play_audio(self.explosion)

    def play_canister(self):
        webapp.play_audio(self.canister)
...
class SmokeRenderer:
    ...
    def add(self, position, rotation, scale):
        ...
        self.instances.extend(self.instance.pack(*position, rx, ry, rz, rw, scale))
...
class Beam:
    def __init__(self, owner, position, velocity):
        ...
        self.rotation = tuple(quat_look_at(velocity, (0.0, 0.0, 1.0)))
        self.life = 200
        self.alive = True
...
class ShipSmoke:
    def __init__(self, position, velocity):
        self.position = position
        self.velocity = velocity
        self.rotation = tuple(random_rotation())
...
class World:
    ...
    def nearby(self, position, radius):
        ...
        return [obj for obj in self.game_objects if type(obj) is not ShipSmoke and glm.distance2(obj.position, position) <= radius2]
...
class Base:
    ...
    def render(self):
        ...
        if self.leaving:
            ...
            text_renderer.line(window.size[0] / 2.0 - 180, window.size[1] / 2.0 + 40, 'press [ENTER] to continue')
...
class Tutorial:
    ...
    def render(self):
        ...
        if self.leaving:
            ...
            text_renderer.line(cx - 180, cy + 40, 'press [ENTER] to continue')
            if window.key_pressed('space'):
                g.total_score += self.space_ship.canisters_collected
                save_score(g.total_score)
                g.scene = Base(self.space_ship.space_ship_model)
            ...
            text_renderer.line(cx - 220, cy + 100 + 100, 'To quit anytime press [ENTER]')
