...
gl = canvas.getContext('webgl2', js.window.JSON.parse('{"antialias":false}'))
callback = js.window.eval(zengl._extern_gl.replace('wasm.allocateUTF8(gl.getParameter(pname))', '0'))
symbols = callback(js.window.Module, gl)
js.window.mergeLibSymbols(symbols)
zengl.init(lambda: None)
...
async def main():
    ...
    class UserInput:
        def __init__(self):
            self.keys = set()
            self.prev_keys = set()
            self.mouse_absolute = [0, 0]
            self.mouse = [0, 0]

        def key_pressed(self, key):
            return key in self.keys and key not in self.prev_keys
    ...
    class Speaker:
        def __init__(self):
            self.intro = pygame.mixer.Sound('data/impact-prelude.ogg')
            self.music = pygame.mixer.Sound('data/beauty-flow.ogg')
            self.hover = pygame.mixer.Sound('data/spaceEngine_002.ogg')
            self.beam = pygame.mixer.Sound('data/laserSmall_003.ogg')
            self.explosion = pygame.mixer.Sound('data/explosionCrunch_001.ogg')
            self.canister = pygame.mixer.Sound('data/forceField_003.ogg')
            self.players = []
        ...
        def update(self):
            self.players = [player for player in self.players if player and player.get_busy()]
    ...
    assets_filename = 'data/assets.pickle'
    ...
    image = ctx.image((window_width, window_height), 'rgba8unorm', samples=4)
    depth = ctx.image((window_width, window_height), 'depth24plus', samples=4)
    ...
    def set_camera(eye, target, upward):
        ...
        camera = zengl.camera(eye, target, upward, fov=60.0, aspect=1.777)
    ...
    class TextRenderer:
        def __init__(self):
            ...
            self.pipeline = ctx.pipeline(
                ...
                ''',
                includes={
                    'screen_size': f'const vec2 screen_size = vec2({window_width}.0, {window_height}.0);',
                },
                layout=[
                    {
                        'name': 'Texture',
    ...
    class SpaceShipControl:
        ...
        def update(self):
            yaw = -1.0 if user_input.key_down('left') else 1.0 if user_input.key_down('right') else 0.0
            pitch = -1.0 if user_input.key_down('up') else 1.0 if user_input.key_down('down') else 0.0
            roll = -1.0 if user_input.key_down('turn_left') else 1.0 if user_input.key_down('turn_right') else 0.0
            yaw += user_input.mouse[0] / 50.0
            pitch += user_input.mouse[1] / 50.0
            self.space_ship.user_input = glm.vec3(yaw, pitch, roll)
            self.space_ship.shooting = user_input.key_down('shoot')
    ...
    class Intro:
        ...
        def render(self):
            ...
            if user_input.key_pressed('space') or user_input.key_pressed('escape'):
            ...
            if self.frame > 2000:
                text_renderer.line(window_width / 2 - 180, 100, 'press [SPACE] to continue')
    ...
    class Base:
        ...
        def render(self):
            ...
            if self.frame % 100 == 0:
                speaker.play_hover()
            self.view = (user_input.mouse_absolute[0] * 0.003, user_input.mouse_absolute[1] * 0.003)
            self.view = min(max(self.view[0], -1.0), 1.0), min(max(self.view[1], -0.5), 0.5)
            self.frame += 1

            if self.leaving:
                text_renderer.line(window_width / 2.0 - 180, window_height / 2.0 + 120, 'Exit the game?')
                text_renderer.line(window_width / 2.0 - 180, window_height / 2.0 + 70, 'press [SPACE] to exit')
                text_renderer.line(window_width / 2.0 - 180, window_height / 2.0 + 40, 'press [ESCAPE] to continue')
                if user_input.key_pressed('space'):
                    g.scene = Outro()
            else:
                text_renderer.line(window_width / 2 - 180, 100, 'press [SPACE] to start')

            if user_input.key_pressed('escape'):
            ...
            if not self.leaving:
                if user_input.key_pressed('left'):
                ...
                if user_input.key_pressed('right'):
                    self.space_ship_model = self.ships[(self.ships.index(self.space_ship_model) + 1) % len(self.ships)]
                    speaker.play_explosion()
                    self.world.add(Explosion((0.0, 0.0, 0.6), 20))
            ...
            text_renderer.line(window_width - 300, window_height - 50, f'Dark Matter: {g.total_score}')
            text_renderer.line(window_width - 300, window_height - 80, f'Unlocked Ships: {self.unlocked_ships}')
    ...
    class Play:
        ...
        def render(self):
            ...
            if not self.leaving and not self.space_ship.alive:
                text_renderer.line(window_width / 2.0 - 180, window_height / 2.0 + 120, 'Game over')
                text_renderer.line(window_width / 2.0 - 180, window_height / 2.0 + 70, 'press [SPACE] to exit')
                if user_input.key_pressed('space'):
            ...
            if self.leaving:
                text_renderer.line(window_width / 2.0 - 180, window_height / 2.0 + 120, 'Abort mission?')
                text_renderer.line(window_width / 2.0 - 180, window_height / 2.0 + 70, 'press [SPACE] to exit')
                text_renderer.line(window_width / 2.0 - 180, window_height / 2.0 + 40, 'press [ESCAPE] to continue')
                if user_input.key_pressed('space'):
                    g.total_score += self.space_ship.canisters_collected
            ...
            text_renderer.line(window_width - 250, window_height - 50, f'Score: {self.space_ship.canisters_collected}')
            text_renderer.line(window_width - 250, window_height - 80, f'Canisters: {sum(1 for x in self.world.game_objects if type(x) is Canister)}')
            text_renderer.line(window_width - 250, window_height - 110, f'Rivals: {sum(1 for x in self.world.game_objects if type(x) is WanderingShip)}')
            text_renderer.render()
    ...
    class Tutorial:
        ...
        def render(self):
            ...
            if user_input.key_pressed('space') and self.hint != 4 and self.hint != 6:
                if self.hint == 3:
                    for _ in range(150):
                        canister = Canister()
            ...
            if self.hint >= 2:
                text_renderer.line(window_width - 250, window_height - 50, f'Score: {self.space_ship.canisters_collected}')
                text_renderer.line(window_width - 250, window_height - 80, f'Canisters: {canisters}')
                text_renderer.line(window_width - 250, window_height - 110, f'Rivals: {rivals}')
    ...
    while True:
        js.window.canvas.width = 1920
        js.window.canvas.height = 1080

        mx, my = pygame.mouse.get_pos()
        mx, my = (mx - window_width / 2.0) * 0.4, (window_height / 2.0 - my) * 0.4
        user_input.mouse_absolute = [mx, my]
        ml = math.sqrt(mx * mx + my * my)
        mx, my = mx / ml, my / ml
        ml = min(max(ml - 5.0, 0.0), 50.0) / 50.0
        ml = ml * ml * 50.0
        mx, my = mx * ml, my * ml
        user_input.prev_keys = user_input.keys.copy()
        user_input.mouse = [mx, my]
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                user_input.keys.add(KEYS.get(event.key, None))

            if event.type == pygame.KEYUP:
                user_input.keys.discard(KEYS.get(event.key, None))

            if event.type == pygame.MOUSEBUTTONDOWN:
                user_input.keys.add(KEYS.get(event.button, None))

            if event.type == pygame.MOUSEBUTTONUP:
                user_input.keys.discard(KEYS.get(event.button, None))
